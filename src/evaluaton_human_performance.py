...
def evaluate_Tissue_Tiles(csvpathTissue: str):
    ...
    for pathologist in ['Pathologist 1', 'Pathologist 2']:
        ...
        # ROC and AUC calculations for multi-class
        fpr = dict()
        tpr = dict()
        roc_auc = dict()
        ...
        plt.figure()
        for i, color in zip(range(len(classes)), ['blue', 'red', 'green']):
        ...
        plt.title(f'ROC for {pathologist}')
        plt.legend(loc="lower right")
        plt.savefig(f"results experiment/ROC_Tissue_Tiles_{pathologist}")
        plt.show()
